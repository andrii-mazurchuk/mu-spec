    ...
    **Closed vocabularies are read from the modules that define them**, never
    ...
    return [
        ...
        tool(
            ...
            "file with, and they MUST NOT be worked at the same time. "
            "Overlap is not an order -- neither waits for the other and "
            "nothing says which goes first -- so it is reported separately "
            "from `follows`, and choosing is the caller's job. "
            "`file_scope` is the other half of that: for each file in the "
            "write set, the other contracts it must eventually serve, with "
            "bodies. Read it before designing anything. A unit is one "
            "contract but a file is not, and whichever unit reaches a file "
            "first decides its shape for all of them. "
            "Also carries the entries "
            "with full bodies, the justification chain, a spine-only read set "
            "of what it depends on elsewhere, cross-cutting entries, "
            "`follows` -- the units this one waits on -- and `size`, for "
            "deciding how many units one session should take. A unit holding "
            "test "
            "modules is one of those: the unit implementing a spec entry "
            "follows the unit holding the scenarios derived from it, which is "
            "how "
        ...
        tool(
            ...
            "Both the cut and the live projection carry `edges` -- what waits "
            "on what, the contract a consumer honours -- and `overlap`, which "
            "units share files and so must not run at the same time. Those "
            "are different relations: an edge is an order, an overlap is not. "
            "`waves` is a view over the edges, with test units shown "
            "immediately before the work they precede.",
            "GET",
        ...
        tool(
            ...
            "a unit's write set is every module implementing ITS OWN "
            "entries, so a file claiming both kinds would put the test file "
            "inside the implementation unit's write set -- letting the agent "
            "edit the tests that judge it. "
